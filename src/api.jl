...
"""
    create_pairing_dictionaries(library_df::DataFrame;
                               seq_col=:PeptideSequence,
                               charge_col=:PrecursorCharge,
                               entrap_col=:EntrapmentGroupId,
                               pair_col=:PrecursorIdx)

Create notebook-style pairing dictionaries for compatibility.

# Returns
- `pair_dict`: Dict mapping (sequence, charge) to pair ID
- `is_original_dict`: Dict mapping (sequence, charge) to original status
"""
function create_pairing_dictionaries(
    ...
    sequences = library_df[!, seq_col]::Vector{String}
    charges = library_df[!, charge_col]::Vector{Int}
    entrap_groups = library_df[!, entrap_col]::Vector{Int}
    pair_ids = library_df[!, pair_col]::Vector{Int}
    
    pair_dict = Dict{Tuple{String, Int}, Int}()
    is_original_dict = Dict{Tuple{String, Int}, Bool}()
    
    for i in 1:nrow(library_df)
        key = (sequences[i], charges[i])
        if !haskey(pair_dict, key)
            pair_dict[key] = pair_ids[i]
            is_original_dict[key] = entrap_groups[i] == 0
        end
    end
...
function run_protein_efdr_analysis(
    ...
    pair_dict, is_original_dict = create_pairing_dictionaries(library_df)
    ...
    protein_no_decoys = calculate_protein_efdr!(
        protein_df, pair_dict, is_original_dict;
        score_col=score_col,
        show_progress=show_progress
    )
